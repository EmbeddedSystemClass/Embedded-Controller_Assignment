...
void LoadBorder(const string title)
{
    //Clear LCD Screen
    lcd.Clear(0xFF003538);
    
    //Top Title
    Label mainTitle(240, 5, title, Label::CENTER, Font16, LCD_COLOR_GREEN);
...
void LoadUpdateingLabels()
{
    ...
    while(1)
    {
        ...
        if (soundResult > 80)
        {
            //Toggles LED
            LEDToggle();
            SoundLabelText.Draw("Warning: Noise!", LCD_COLOR_WHITE); //Redraw Label
        }
        else
        {
            led = 0;
